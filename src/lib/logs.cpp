...
void  _log_error  (Return_code _code, const char* file, const char* func, const int line) {
    ...
    case LIB_ERR:
        print_log_time();
        fprintf (log_file, "lib error in file %s in function %s (line %d)\n", file, func, line); break;

    case CRITICAL:
        print_log_time();
        fprintf (log_file, "critical error in file %s in function %s (line %d)\n", file, func, line); break;

    case SPECIAL_CASE:
        print_log_time();
        fprintf (log_file, "special case in file %s in function %s (line %d)\n", file, func, line); break;
